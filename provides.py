...
class KapacitorProvides(RelationBase):
    scope = scopes.UNIT

    @hook('{provides:kapacitor}-relation-joined')
    def joined(self):
        conv = self.conversation()
        conv.remove_state('{relation_name}.broken')
        conv.set_state('{relation_name}.connected')

    @hook('{provides:kapacitor}-relation-changed')
    def changed(self):
        conv = self.conversation()
        conv.remove_state('{relation_name}.departed')
        conv.set_state('{relation_name}.available')

    @hook('{provides:kapacitor}-relation-broken')
    def broken(self):
        conv = self.conversation()
